...
import unitest/internal/runner.{
  ...
  type TestResult, Failed, Passed, Platform, PoolResult, Ran, Report, Run,
  RunError, RuntimeSkip, Skip, Skipped,
}
...
pub fn execute_passing_tests_counts_passed_test() {
  ...
  let now_ms = fn() { 100 }
  let platform = make_sequential_platform(fn(_t) { Ran }, now_ms)
...
pub fn execute_failing_test_counts_failure_test() {
  ...
  let platform =
    make_sequential_platform(
      fn(_t) { RunError(test_failure("assertion failed")) },
      now_ms,
    )
...
pub fn execute_runtime_skip_counts_skipped_test() {
  ...
  let now_ms = fn() { 100 }
  let platform = make_sequential_platform(fn(_t) { RuntimeSkip }, now_ms)
...
pub fn execute_pooled_returns_results_in_completion_order_test() {
  ...
  let platform =
    make_pooled_platform(fn(_groups, _workers) {
      send_pool_result(PoolResult(t2, Ran, 3))
      send_pool_result(PoolResult(t1, Ran, 7))
      send_pool_result(PoolResult(t3, Ran, 1))
    })
...
pub fn execute_pooled_mixed_run_skip_test() {
  ...
  let t3 = make_test("m", "c_test", [])
  let plan = [Run(t1), Skip(t2), Run(t3)]

  let platform =
    make_pooled_platform(fn(groups, _workers) {
      list.each(groups, fn(group) {
        list.each(group, fn(t) { send_pool_result(PoolResult(t, Ran, 5)) })
      })
    })
...
pub fn execute_pooled_failed_test_in_pool_test() {
  ...
  let platform =
    make_pooled_platform(fn(_groups, _workers) {
      send_pool_result(PoolResult(t1, RunError(test_failure("crash bang")), 12))
    })
...
pub fn execute_pooled_mixed_pass_fail_skip_test() {
  ...
  let plan = [Run(t1), Run(t2), Skip(t3), Run(t4)]

  let platform =
    make_pooled_platform(fn(_groups, _workers) {
      send_pool_result(PoolResult(t1, Ran, 5))
      send_pool_result(PoolResult(
        t2,
        RunError(test_failure("assertion failed")),
        10,
      ))
      send_pool_result(PoolResult(t4, RuntimeSkip, 2))
    })
...
pub fn execute_pooled_emits_skip_callbacks_test() {
  ...
  assert list.contains(outcomes, Skipped)
